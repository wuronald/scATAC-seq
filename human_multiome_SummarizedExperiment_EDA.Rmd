...
sum(is.na(fdr)) # 761
```
Here we observe 761 NAs in FDR column total among all the groups within PIMO_up_status. Recall, we start with a total of 167905 features unfiltered.
167905 - 761 = 167144

Below is a breakdown for each group:
167905 - 35 = 167870
167905 - 49 = 167856
167905 - 677 = 167228
...
The number of features in the MA plots (167905) doesn't match the total features found in the Granges tables, which differ for each PIMO category.
Oddly, when we intersect the GRanges from the three categories, we end up with 166949 features. When the PIMOup GRanges (167228) is intersected with itself, we end up with 166949 as well, suggesting that some features are lost. It's unclear where the features went, and we also checked for NAs.
