#!/bin/bash
#SBATCH --job-name=run_01_human_multiome_createArrowFiles
#SBATCH --output=run_01_human_multiome_createArrowFiles_%j.out
#SBATCH --error=run_01_human_multiome_createArrowFiles_%j.err
...
# input files setup:
parent_dir <- here::here("data", "human_multiome")
...
projMulti1 <- ArchRProject(
  ...
  outputDirectory = "human_multiome",
...
projMulti2 <- subsetArchRProject(ArchRProj = projMulti1, cells = getCellNames(projMulti1)[cellsToKeep], outputDirectory = "human_multiome", force = TRUE)
...
saveArchRProject(ArchRProj = projMulti2, outputDirectory = "human_multiome", load = TRUE)
