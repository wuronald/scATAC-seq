#!/bin/bash
#SBATCH --job-name=run_08_mouse_multiome_chromVarDeviations
#SBATCH --output=run_08_mouse_multiome_chromVarDeviations_%j.out
#SBATCH --error=run_08_mouse_multiome_chromVarDeviations_%j.err
...
# Example usage:
# sbatch scripts/run_08_mouse_multiome_chromVarDeviations.sh homer PIMO_up_status
# sbatch scripts/run_08_mouse_multiome_chromVarDeviations.sh cisbp hybrid_pair "SOX,HIF,ARNT,NF1,NFI"
...
outDir <- here(paste0("mouse_multiome_harmony_merged_malig_peak_subset/chromVarDeviations_", motifSet, "_", groupBy, "/"))
...
if ("Harmony_LSI_Combined_default" %in% names(proj@reducedDims)) {
    print("ReducedDims 'Harmony_LSI_Combined_default' exists.")
} else {
    stop("ReducedDims 'Harmony_LSI_Combined_default' does not exist in the ArchR project.")
}

proj <- addImputeWeights(
    proj,
    reducedDims = "Harmony_LSI_Combined_default"
)
...
if (length(markerMotifs) == 0) {
    ...
    p_umap <- plotEmbedding(
        ...
        embedding = "UMAP_Harmony_LSI_Combined_default",
        imputeWeights = getImputeWeights(proj)
    )
...
g4 <- plotEmbedding(proj, 
colorBy = "cellColData", 
name = "Azimuth_class", 
embedding = "UMAP_Harmony_LSI_Combined_default")

g5 <- plotEmbedding(proj, 
colorBy = "cellColData", 
name = "Azimuth_subclass", 
embedding = "UMAP_Harmony_LSI_Combined_default")

plotPDF(list(g1, g2, g3, g4, g5), name = paste0(filePrefix, "-Example-GeneExpression-and-Groupings_", motifSet), width = 5, height = 5, ArchRProj = proj, addDOC = TRUE)
